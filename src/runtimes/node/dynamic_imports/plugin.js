...
const getDynamicImportsPlugin = ({ basePath, includedPaths, moduleNames, resolveDynamicImports, srcDir }) => ({
  ...
  setup(build) {
    ...
    build.onDynamicImport({}, async (args) => {
      ...
      // Also don't parse the expression if we're not interested in resolving
      // dynamic imports.
      if (!basePath || !resolveDynamicImports) {
        return
      }
